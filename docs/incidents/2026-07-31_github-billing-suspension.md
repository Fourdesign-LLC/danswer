...
### 5. 再発防止の実装（2026-08-01）

「3日間気づけなかった」ことへの対応として、`Fourdesign-LLC/stock-viewer` に検知を追加した。

**PR: [stock-viewer#2](https://github.com/Fourdesign-LLC/stock-viewer/pull/2)**
（ブランチ `claude/snapshot-batch-failure-alert`）

- `snapshot-batch.yml` に `notify` ジョブを追加。失敗時は必ず通知し、成功時は実処理が
  走った日（月末・手動実行）のみ通知する。平常時は月1通に収まり、**月末に成功通知が
  来ないこと自体が欠測のサイン**になる
- `SLACK_WEBHOOK_URL` 未設定時はジョブを落とす（通知経路の死亡を無音にしない）
- `lib/snapshot/generate.ts` のメーカー単位 `catch` が失敗を握りつぶしており、
  **4メーカー全滅でも exit 0** だった。通知を足すだけでは「✅完了」が出てしまうため、
  失敗を集約して非ゼロ終了するよう修正。実行される処理内容は変えていない

検証: YAMLパース / `bash -n` / `jq` 出力の実行確認 / `npx tsc --noEmit` exit 0。

**マージ前に `SLACK_WEBHOOK_URL` シークレットの登録が必要。**

#### この通知でカバーできない範囲（重要）

**今回の 7/29〜31 の事象そのもの＝ジョブがランナーに配られないケースでは、この通知は
発火しない。** ステップが1つも実行されないため、同一ワークフロー内のどのジョブからも
検知できない（`if: failure()` の別ジョブでも同じ）。

この系統は **GitHub の外側からの死活監視**でしか拾えない。既存の日次ヘルスチェック
（Slack `#_information_system` に毎日 04:58 JST 前後で投稿）に「前月末のスナップショットが
DBに存在するか」を1項目足すのが最小の対応。**未実施。**
